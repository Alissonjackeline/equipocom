                            ...
                            @foreach ($returns as $return)
                                <tr>
                                    ...
                                    <td>
                                        @if($return->equipment)
                                            <div class="mb-1">
                                                ...
                                                <span class="info-value">{{ $return->equipment->equipmentType->Name ?? 'N/A' }}</span>
                                            </div>
                                            <div class="mb-1">
                                    ...
                                    <td class="text-center">
                                        @php
                                            $devolvioText = $return->Devolucion ? 'Sí' : 'No';
                                            $devolvioColor = $return->Devolucion ? 'success' : 'danger';
                                        @endphp
                                        <span class="badge bg-{{ $devolvioColor }}">
                                            {{ $devolvioText }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">
                                        @php
                                            $estadoText = match($return->equipment->status ?? 0) {
                                                1 => 'Disponible',
                                                2 => 'Por preparar',
                                                3 => 'En uso',
                                                4 => 'Observación',
                                            ...
                                            $estadoColor = match($return->equipment->status ?? 0) {
                                                1 => 'success',
                                                2 => 'warning',
                                                3 => 'info',
                                                4 => 'danger',
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <button class="btn btn-warning btn-sm action-btn" title="Editar"
                                                data-bs-toggle="modal" data-bs-target="#modalEditar{{ $return->id }}">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </button>
                                            <button class="btn btn-danger btn-sm action-btn" title="Eliminar"
                                                data-bs-toggle="modal" data-bs-target="#modalEliminar{{ $return->id }}">
                                ...
                                <!-- Modal para Editar -->
                                <x-modal-base id="modalEditar{{ $return->id }}" title="Editar Devolución" size="modal-lg">
                                    <form action="{{ route('devolucion.update', $return->id) }}" method="POST" enctype="multipart/form-data" class="row">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="col-md-12 pt-2">
                                            ...
                                            <select name="estado" class="form-select" required>
                                                <option value="1" {{ $return->equipment->status == 1 ? 'selected' : '' }}>Disponible</option>
                                                <option value="2" {{ $return->equipment->status == 2 ? 'selected' : '' }}>Por preparar</option>
                                                <option value="4" {{ $return->equipment->status == 4 ? 'selected' : '' }}>Observación</option>
                                                <option value="5" {{ $return->equipment->status == 5 ? 'selected' : '' }}>R Pendiente</option>
                                                <option value="6" {{ $return->equipment->status == 6 ? 'selected' : '' }}>No devuelto</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-12 pt-2">
                                            ...
                                            <textarea name="comentario" class="form-control" rows="3" 
                                                      placeholder="Observaciones sobre la devolución">{{ $return->Comment }}</textarea>
                                        </div>
                                        ...
                                        <div class="col-md-6 pt-2">
                                            ...
                                            @if($return->Document)
                                                <small class="text-success">
                                                    <i class="fa-solid fa-file"></i> Documento actual disponible
                                                </small>
                                            @endif
                                        </div>
                                    </form>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <p><strong>Equipo:</strong> {{ $return->equipment->CodigoPatri ?? 'N/A' }}</p>
                                                <p><strong>Fecha:</strong> {{ $return->Date->format('d/m/Y H:i') }}</p>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                                                <form action="{{ route('devolucion.destroy', $return->id) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                <!-- Modales para documentos, imágenes y comentarios -->
                                @if ($return->Document)
                                    <x-btn-documento id="modalDocu{{ $return->id }}" 
                                        title="DOCUMENTO - Devolución #{{ $return->id }}" size="modal-lg">
                                        <embed src="{{ asset('storage/' . $return->Document) }}" type="application/pdf" width="100%" height="500px" />
                                    </x-btn-documento>
                                @endif
                                ...
                                @if ($return->Image)
                                    <x-btn-imagen id="modalImg{{ $return->id }}" 
                                        ...
                                        <img src="{{ asset('storage/' . $return->Image) }}" class="img-fluid" alt="Imagen de la devolución">
                                    </x-btn-imagen>
                                @endif
